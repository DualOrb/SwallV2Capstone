//! Copy of `wayland-server-0.31.1/src/socket.rs` or [wayland_server::ListeningSocket] modified to be async
use std::{
    env,
    ffi::{OsStr, OsString},
    fs::{self, File},
...
use smithay::reexports::{
    rustix::fs::{flock, FlockOperation},
    wayland_server::BindError,
};
...
impl ListeningSocket {
    ...
    pub fn bind<S: AsRef<OsStr>>(socket_name: S) -> Result<Self, BindError> {
        let runtime_dir: PathBuf = env::var("XDG_RUNTIME_DIR")
            .map_err(|_| BindError::RuntimeDirNotSet)?
            .into();
